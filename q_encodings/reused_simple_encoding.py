# Irfansha Shaik, 18.10.2021, Linz.
...
'''
In Organic synthesis benchmarks, the number of parameters for each reaction differ from 4 to 22.
Thus it is possible to reuse the allocated parameter variables upto two steps (can be more, for now 2)
i.e., by generating one transition function from forward and the second transition function from backwards.
'''

class ReusedSimpleEncoding:
  ...
  def generate_quantifier_blocks(self):
    ...
    for i in range(self.plan_length):
      ...
      for j in range(len(self.parameter_variables[i])):
        cur_parameter_variables = self.parameter_variables[i][j]
        for cur_var in cur_parameter_variables:
          # we only append in not provided already:
          if (cur_var not in first_layer_variables):
            first_layer_variables.append(cur_var)
  ...
  def generate_goal_gate(self):
    ...
    for non_static_predicate in self.tfunc.probleminfo.non_static_predicates:
      ...
      cur_nonstatic_variable = self.non_static_variables[self.plan_length][self.tfunc.probleminfo.non_static_predicates.index(non_static_predicate)]
  ...
  def __init__(self, tfunc):
    ...
    # we use the plan_length as make span and generate extra transition function steps:
    self.make_span = tfunc.parsed_instance.args.plan_length
    # For now, we generate twice the make span reusing parameter variables:
    self.plan_length = 2*self.make_span


    # Generating k sets of action and parameter variables:
    self.action_variables = []
    self.parameter_variables = []
    for i in range(self.make_span):
      ...
      # Difference here (from Simple encoding) is we generate twice:
      second_single_step_action_vars = self.encoding_variables.get_vars(tfunc.probleminfo.num_action_variables)
      self.action_variables.append(second_single_step_action_vars)
      ...
      # For the first action parameters, we append the parameter list as it is:
      self.parameter_variables.append(single_step_parameter_variable_list)
      # For second transition, we append the reversed parameter list:
      reversed_parameter_variable_list = []
      for var_list in single_step_parameter_variable_list:
        # Inserting in the front to reverse the next time step parameters:
        reversed_parameter_variable_list.insert(0, var_list)
      self.parameter_variables.append(reversed_parameter_variable_list)
